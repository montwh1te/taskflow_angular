...
const DEFAULT_TASKS: Task[] = [
  { id: 101, projectId: 1, title: 'Configurar ambiente de dev', description: 'Instalar todas as dependências.', status: 'Concluído', dueDate: new Date('2025-10-18') },
  { id: 102, projectId: 1, title: 'Criar tela de login', description: 'Desenvolver o componente de autenticação.', status: 'Em Andamento', dueDate: new Date('2025-10-17') },
  { id: 103, projectId: 2, title: 'Pesquisa bibliográfica', description: 'Encontrar artigos relevantes.', status: 'A Fazer', dueDate: new Date('2025-10-16') },
];
...
export class MockApiService {
  ...
  updateProject(id: number, changes: { title?: string; description?: string }): Observable<Project | null> {
  ...
  createTask(task: { projectId: number; title: string; description: string; status: string; dueDate: Date }) {
    const newId = this.tasks.length ? Math.max(...this.tasks.map(t => t.id)) + 1 : 101;
    const newTask: Task = {
      id: newId,
  ...
  updateTask(id: number, changes: { title?: string; description?: string; status?: string; dueDate?: Date }): Observable<Task | null> {
